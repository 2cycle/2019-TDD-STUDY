...
public class MemberControllerTest {
  ...
  private MockMvc mockMvc; //기반 테스트
  ...
  public void setUp() {
    ...
    /**
     * {class-name}/{method-name}는 테스트 클래스의 메서드 이름 기반으로 디렉토리 경로를 설정해서 snippets을 생성
     * target/generated-snippets/member-controller-test/get_member
     * modifier인 prettyyprint()가 출력을 예쁘게 만들어줌.
     */
  ...
  public void get_member() throws Exception {
    mockMvc.perform(get("/members/{id}", 1L)
        ...
        .andDo(document.document(   //.andDo(document("document-identifier 로 사용가능
            pathParameters( //parameter를 정의
                parameterWithName("id").description("Member's id")
            ),
            responseFields( //responsefiled에 대해 문서를 정의하는 것
